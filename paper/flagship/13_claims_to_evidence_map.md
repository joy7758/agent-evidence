...
| the minimal boundary is stable across more than one scenario | 防止边界被单一 demo 绑死 | 当前已有 5 个 scenario families，`15_validation_appendix_index.md` 已把它们压成 reviewer-facing corpus index，且 supplementary external-context specimen `external_context/data_space_metadata_update.valid.json` 已在当前 validator path 下通过 | 仍缺更多 repo 外场景与更强 object-operation diversity；该 supplementary evidence 支持旗舰主线，但 not counted in canonical B1 counts | additional external-context specimens | main paper |
...
| the boundary can be checked by more than one reasoning path | 降低单实现闭环风险 | reference validator、independent checker prototype、`14_checker_comparison_note.md`、19-file pass/fail comparison、run archive、`assets/run_archive/json/`，以及 supplementary repo-local second checker surface `second_checker/` | 仍缺更强独立性与第三方实现；该 supplementary checker surface 支持旗舰主线，但 not counted in canonical B1 counts | third-party or different-language checker | main paper / artifact package |
...
补充说明：`external_context/` 与 `second_checker/` 现在都是 supplementary surfaces。它们支持 flagship line，但 not counted in canonical B1 minimal-frozen counts。
