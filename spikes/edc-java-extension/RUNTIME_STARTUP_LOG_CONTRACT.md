...
当前最小契约里，`AgentEvidenceEdcExtension` 在 startup 阶段就必须明确输出：
...
- 日志中出现 `Using agent-evidence output directory '...'`

如果是 `filesystem`，后续 writer 运行时还应能把写入目标路径打到 debug 日志中。
startup smoke 当前只要求 startup 阶段先把 output directory 打出来，不要求等待 writer debug 日志。
...
4. `Using agent-evidence output directory '...'`
5. `Registered control-plane event subscribers for agent-evidence spike`
6. `Runtime ... ready`
...
- 看到 `Using agent-evidence output directory '...'`
