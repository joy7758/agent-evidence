...
| 2026-05-23 | opentelemetry-to-eeoap-adapter-prototype | adapter | Added a local OpenTelemetry-style GenAI trace to EEOAP v0.1 adapter prototype with local fixtures, diagnostics, and mapping docs. | Agents can convert bounded local telemetry fixtures into operation accountability statements without creating a new profile. | `ruff check .`, `ruff format --check .`, and `pytest`. | Preserves EEOAP schema integrity by reusing `agent-evidence validate-profile` and representing trace/span links as references and artifacts. | Expand fixture coverage only after the minimal adapter path remains validator-clean. |
