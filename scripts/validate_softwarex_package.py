...
REQUIRED_FILES = [
    ...
    "repo/src/agent_evidence/oap.py",
    ...
    "paper/softwarex_osp_template_manuscript.docx",
    "paper/softwarex_osp_template_manuscript.pdf",
...
REQUIRED_PAPER_PHRASES = [
    ...
    "Permanent GitHub link to code/repository used for this code version",
    "Apache License 2.0",
    "## Main text",
    ...
    "### 5. Conclusions",
    "`scripts/reproduce_paper.sh`",
    "OpenTelemetry trace data can be transformed into an Execution Evidence and Operation Accountability Profile",
    "The intended users are researchers, tool builders, and developers",
    "The file `examples/io_trace_to_eeoap.json` gives a compact input-output example.",
    "The evaluation focuses on structural correctness and reproducibility",
    "## Acknowledgements",
...
REQUIRED_COVER_LETTER_PHRASES = [
    ...
    "schema definition",
    "trace-to-evidence conversion workflow",
    "validation framework",
    "baseline comparison",
    "reproducibility scripts",
    "scoped as an original SoftwareX research software article",
...
def check_docx_template_clean(archive: ZipFile, path: str) -> tuple[bool, list[str]]:
    findings: list[str] = []
    markers = [
        "Before you complete this template",
        "Please fill in this column",
        "delete this line",
        "SoftwareX article template Version",
    ...
    with ZipFile(BytesIO(payload)) as docx_archive:
        document_xml = docx_archive.read("word/document.xml").decode("utf-8", errors="ignore")
    ...
    with ZipFile(args.input) as archive:
        ...
        readme_ok = all(
            ...
            for phrase in [
                ...
                "paper/data_availability_statement.md",
                "Scope limits are stated in the manuscript evaluation-scope section",
                "repo/src/agent_evidence/",
            ]
        )
        ...
        docx_clean, docx_findings = check_docx_template_clean(
            archive, "paper/softwarex_osp_template_manuscript.docx"
        )
        results["official_template_docx_clean"] = docx_clean
        results["official_template_docx_findings"] = docx_findings
